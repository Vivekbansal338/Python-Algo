"""
================================================================================
V4 SECTOR BACKTEST ENGINE (PHASE 1)
================================================================================
Validates Sector Ranking logic with high-fidelity historical simulation.

Features:
- Daily & 5-Min Data Alignment.
- Real-time Breadth simulation (Calculates VWAP per stock).
- Nifty Vector (Market Gravity) integration.
- Direct use of production SectorScorer.
...
class SectorBacktester:
    ...
    def _validate_data_sufficiency(self, start_date: datetime.date) -> bool:
        ...
        logger.info(f"✅ Data Sufficiency Check Passed.")
        return True
    ...
    def _scan_stocks(self, selected_sectors: List[str], final_scores: List[SectorScore], timestamp: datetime) -> List[Dict]:
        ...
        # STRICT TIME GUARD: No scanning/entries after Cutoff (14:05)
        if timestamp.time() >= config.ENTRY_CUTOFF_TIME:
            return []
        ...
        for sym, sec_sym in target_stocks:
            ...
            signal = self.grader.calculate_grade(hma_align, rvol, stoch_k, sec_info.rank if sec_info else 10, 0.0, "MAIN", 50)
...
if __name__ == "__main__":
    ...
    # engine.run_backtest(datetime(2025, 3, 1).date(), datetime(2026, 1, 9).date())
