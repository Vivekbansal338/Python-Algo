...
class AdvancedBacktestAnalyzer:
    ...
    def _core_equity(self) -> Dict[str, Any]:
        ...
        curve = [ini]
        for pnl in self.trade_df.get("realized_pnl", pd.Series(dtype=float)).tolist():
    ...
    def direction_matrix(self) -> Dict[str, Any]:
        ...
        for i, s, g in itertools.product(["UP", "DOWN"], ["UP", "DOWN"], ["UP", "DOWN"]):
            sub = valid[(valid["index_dir"] == i) & (valid["sector_dir"] == s) & (valid["signal_dir"] == g)]
            ...
            rows.append(
                {
                    ...
                    "pnl": round(float(sub["realized_pnl"].sum()) if trades else 0.0, 2),
                }
            )
        aligned = valid[(valid["index_dir"] == valid["sector_dir"]) & (valid["sector_dir"] == valid["signal_dir"])]
        return {
            "rows": rows,
    ...
    def capital_utilization(self) -> Dict[str, Any]:
        ...
        return {"rows": sorted(rows, key=lambda x: x["peak_deployed_notional"], reverse=True), "summary": summary}
    ...
    def build_analysis(self) -> Dict[str, Any]:
        return {
            ...
            "blockers_analysis": self._blockers(),
    ...
    def print_report(self, top: int = 10) -> Dict[str, Any]:
        ...
        blockers = out["blockers_analysis"]
        top_dec = sorted(blockers.get("decision_breakdown_normalized", {}).items(), key=lambda x: x[1], reverse=True)[:top]
        top_risk = sorted(blockers.get("risk_reason_breakdown_normalized", {}).items(), key=lambda x: x[1], reverse=True)[:top]
        self._print_table("Top Decision Reasons", [{"reason": k, "count": v} for k, v in top_dec], [("reason", "Reason"), ("count", "Count")])
        self._print_table("Top Risk Reasons", [{"reason": k, "count": v} for k, v in top_risk], [("reason", "Reason"), ("count", "Count")])

        self._print_table(
            "Index-Sector-Signal Matrix",
            out["direction_matrix_index_sector_signal"].get("rows", []),
            [("index_dir", "Index"), ("sector_dir", "Sector"), ("signal_dir", "Signal"), ("trades", "Trades"), ("win_rate_pct", "WinRate%"), ("pnl", "PnL")],
        )
        ...
        self._print_table(
            f"Top {top} Days by Peak Deployed",
            out["capital_utilization"].get("rows", []),
            [("day", "Day"), ("trades", "Trades"), ("start_equity", "StartEq"), ("day_pnl", "DayPnL"), ("peak_deployed_notional", "PeakDeploy"), ("peak_utilization_pct", "PeakUtil%")],
            max_rows=top,
        )
        ...
        self._print_table(
            ...
            first.get("rows", []),
            [("first_trade_outcome", "FirstOutcome"), ("days", "Days"), ("day_profit_rate_pct", "DayProfit%"), ("avg_day_pnl", "AvgDayPnL")],
        )
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--export", type=str, default=None, help="Export analysis JSON")
...
def main() -> int:
    ...
    try:
        ...
        if args.export:
            out = AdvancedBacktestAnalyzer.export_json(analysis, args.export)
