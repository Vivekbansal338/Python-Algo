...
class SectorScore:
    symbol: str
    price: float = 0.0
    change_pct: float = 0.0  # V5: Now Daily Change (vs Prev Close)
    structural_rs: float = 0.0
    shortterm_rs: float = 0.0
    intraday_rs: float = 0.0  # V5: Now Daily RS (vs Prev Close)
...
class SectorScorer:
    ...
    def select_top_n(self, ranked_sectors: List[SectorScore]) -> List[str]:
        ...
        for i in range(min(n, len(ranked_sectors))):
            # Only select if it has a directional bias
