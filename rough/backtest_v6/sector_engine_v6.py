...
class SectorBacktesterV6:
    ...
    def _scan_tradeable_stocks(self, ts: pd.Timestamp, baselines: Dict[str, Dict[str, float]]):
        ...
        for symbol in candidates:
            ...
            if can_enter and passed and signal.grade in ("A+", "A"):
                self._process_entry(signal, curr_price, atr, ts)

    def _process_entry(self, signal: StockSignal, ltp: float, atr: float, ts: pd.Timestamp):
        ...
        allowed, _ = self.risk.can_open_new_trade(signal.symbol, signal.sector)
        if not allowed:
            return
        ...
        risk_per_share = abs(ltp - stop_price)
        target_1 = ltp + (risk_per_share * config.TARGET_1_MULT) if signal.direction == "LONG" else ltp - (
            risk_per_share * config.TARGET_1_MULT
        )
    ...
    def _run_day(self, day: date) -> DayResult:
        ...
        for ts, nifty_row in day_nifty.iterrows():
            ...
            if self.current_playbook == "MAIN" and not kill_halt and not self.safety.is_halted:
                self._scan_tradeable_stocks(ts, baselines)
            else:
                self.active_signals = []
