"""
===============================================================================
V6.7 SECTOR BACKTEST ENGINE — ENHANCED SIGNAL QUALITY GATES
===============================================================================
...
# Enable `python backtest_v6/sector_engine_v6.7.py` from repo root.
...
CROSS_FRESHNESS_BARS = 15       # Max bars since last HMA9/HMA21 crossover (~75 min on 5m)

# Gate 3 — Price-HMA alignment (no threshold; structural check only)

# Gate 4 — HMA separation (anti-chop)
MIN_HMA_SEP_ATR_FRAC = 0.15    # Min |HMA9−HMA21| as fraction of 5m ATR
...
class SectorBacktesterV6:
    """
    V6.7 backtester — Enhanced signal quality gates on top of V6.6.
    ...
    def __init__(
        ...
        self.history_dir = self.backtest_root / "history_v6.7"
    ...
    def _get_hma_alignment_v67(
        ...
        # ── Current HMA values ──
        hma9 = calculate_hma(series, 9)
        hma21 = calculate_hma(series, 21)
        ...
        hma9_prev = self._hma_at_offset(series, 9, SLOPE_LOOKBACK)
        hma21_prev = self._hma_at_offset(series, 21, SLOPE_LOOKBACK)
        ...
        for offset in range(1, max_scan):
            h9 = self._hma_at_offset(series, 9, offset)
            h21 = self._hma_at_offset(series, 21, offset)
            if h9 <= 0 or h21 <= 0:
                break
        ...
        h9_old = self._hma_at_offset(series, 9, DIVERGENCE_LOOKBACK)
        h21_old = self._hma_at_offset(series, 21, DIVERGENCE_LOOKBACK)
    ...
    def _print_grand_summary(self):
        ...
        print("\n" + "=" * 80)
        print("V6.7 BACKTEST SUMMARY (ENHANCED SIGNAL QUALITY GATES)")
        print("-" * 80)
    ...
    def export_trades(self, filepath: Optional[Path] = None):
        out_path = Path(filepath) if filepath else (self.data_root / "backtest_trades_v6.7.csv")
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="V6.7 sector backtest engine (enhanced signal quality gates)")
