"""
===============================================================================
V6.5 SECTOR BACKTEST ENGINE — HARD ALIGNMENT FILTER (OPTIMIZED)
===============================================================================
Backtest engine based on V6.2 with a hard 3-way alignment filter that blocks
any trade where index direction, sector bias, and signal direction are not
fully aligned:  only UP,UP,UP or DOWN,DOWN,DOWN trades are executed.

Key difference from V6.3 (MINIMAX):
- V6.3 tried to *prevent* misalignment by changing sector scoring weights
  (nifty 30→150, breadth 40→20, threshold ±10→±20).
- V6.4/V6.5 keep V6.2's ORIGINAL weights and scoring UNCHANGED.  Instead, they add
  a simple hard filter: after the signal is graded, check if index direction
  (derived from nifty_pct) matches sector bias AND signal direction.  If not,
  the signal is demoted to grade C and never executed.

Changes from V6.2:
- 5.1  Hard 3-Way Alignment Filter — Before entry, check:
       index_dir == UP  AND  sec_bias == LONG  AND  signal == LONG  → ALLOW
       index_dir == DOWN AND  sec_bias == SHORT AND  signal == SHORT → ALLOW
       Everything else → grade demoted to C (ALIGNMENT_BLOCKED)
- 5.2  Direction Matrix Tracking — Every trade records index_direction,
       sector_bias, and signal_direction for post-hoc alignment analysis.
- 5.3  Alignment Stats in Summary — Grand summary prints alignment matrix
       and blocked trade stats.
- 5.4  Performance Optimization (V6.5) — Reduce redundant indicator work and
       cache per-bar/per-day data paths without changing trade logic/results.

Unchanged from V6.2 (ALL weights, thresholds, scoring identical):
...
- 2.8  Exit Reason Taxonomy (STOP_HARD, STOP_TRAIL, TARGET1_FULL, FORCE_EXIT,
       SAFETY_HALT)
- Sector scoring weights and bias threshold (ORIGINAL brain.py SectorScorer)
...
# Enable `python backtest_v6/sector_engine_v6.5.py` from repo root.
...
class SectorBacktesterV6:
    """
    V6.5 backtester using V6.2 as base with a hard 3-way alignment filter:
    only UP,UP,UP (Index UP + Sector LONG + Signal LONG) or
    DOWN,DOWN,DOWN (Index DOWN + Sector SHORT + Signal SHORT) trades pass.
    All sector scoring weights remain identical to V6.2 (original brain.py).
    Direction matrix tracked per trade for post-hoc analysis.
    """
    ...
    def __init__(
        ...
        self.history_dir = self.backtest_root / "history_v6.5"
    ...
    def _is_aligned(index_dir: str, sec_bias: str, signal_dir: str) -> bool:
        """Check if index direction, sector bias, and signal direction are
        fully aligned: UP,LONG,LONG or DOWN,SHORT,SHORT."""
    ...
    def _capture_config_snapshot(self) -> Dict[str, Any]:
        ...
        # V6.4: hard alignment filter (no weight changes)
    ...
    def _get_daily_symbol_metrics(self, symbol: str, day: date) -> Optional[Dict[str, Any]]:
        """Per-day immutable metrics for one symbol (cached)."""
        key = (symbol, day)
        ...
        if len(closes) < atr_period + 1:
            ...
            # Same ATR value as full-history call, with O(period) work.
    ...
    def _get_recent_rvol(self, symbol: str, intra_pos: int) -> float:
        ...
        if end < 20:
            return 0.0
        recent = intra_np["volume"][end - 20:end]
    ...
    def _scan_tradeable_stocks(
        ...
        for scan_order, symbol in enumerate(candidates, start=1):
            ...
            hma_align = self._get_hma_alignment(symbol, curr_price, intra_pos, closes)
            ...
            # ── V6.4: Hard 3-way alignment filter ──
            # After existing sector-vs-signal check, also verify index direction
            # matches.  Only UP,LONG,LONG and DOWN,SHORT,SHORT are allowed.
            if signal.grade in ("A+", "A"):
                index_dir = self._get_index_direction(self.nifty_pct)
                if not self._is_aligned(index_dir, sec_bias, signal.direction):
                    signal.grade = "C"
                    signal.reasons.append(
                        f"ALIGNMENT_BLOCKED: Index={index_dir} Sector={sec_bias} Signal={signal.direction}"
                    )
    ...
    def _process_entry(
        ...
        # V6.4: Capture index direction at this moment
    ...
    def _calculate_chandelier(self, trade: BacktestTrade, intra_pos: int) -> float:
        ...
        # Rolling 5-minute ATR over TRAIL_LOOKBACK_BARS (same value as full-prefix ATR).
    ...
    def _run_day(self, day: date) -> DayResult:
        ...
        for ts, nifty_row in day_nifty.iterrows():
            # Cache row lookups within a bar; clear between bars.
            self._intra_row_cache.clear()
    ...
    def run_backtest(self, start_date: date, end_date: date):
        ...
        try:
            ...
            for idx, day in enumerate(trading_days, start=1):
                ...
                # Incremental persistence (optional cadence).
    ...
    def _print_grand_summary(self):
        ...
        print("\n" + "=" * 80)
        print("V6.5 BACKTEST SUMMARY (HARD ALIGNMENT FILTER, OPTIMIZED)")
        print("-" * 80)
        ...
        if self.trade_history:
            ...
            # V6.4/V6.5: Hard Alignment Filter — Direction Matrix
            print(f"\nAlignment Filter (V6.4/V6.5 — Hard Block):")
            print(f"  Alignment Blocked: {self.alignment_blocked_count} A/A+ signals demoted to C")
    ...
    def export_trades(self, filepath: Optional[Path] = None):
        out_path = Path(filepath) if filepath else (self.data_root / "backtest_trades_v6.5.csv")
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="V6.5 sector backtest engine (hard alignment filter, optimized)")
