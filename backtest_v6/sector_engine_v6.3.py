"""
===============================================================================
V6.3 SECTOR BACKTEST ENGINE — MINIMAX ALIGNMENT OPTIMIZATION
===============================================================================
Backtest engine based on V6.2 with MINIMAX-optimized sector scoring to ensure
only ALIGNED trades are taken: UP,UP,UP (Index UP + Sector LONG + Signal LONG)
or DOWN,DOWN,DOWN (Index DOWN + Sector SHORT + Signal SHORT).

Changes from V6.2:
- 4.1  MINIMAX Sector Weight Override — Nifty weight raised from 30 to 150,
       breadth weight lowered from 40 to 20.  High nifty weight mathematically
       forces composite score sign to align with index direction, eliminating
       mismatched trades WITHOUT complex post-filters.
- 4.2  Bias Threshold Raised — ±10 → ±20.  Prevents over-sensitive bias
       triggers on tiny nifty movements (< 0.1%).
- 4.3  Local Scoring Override — Composite score + bias computed locally in the
       engine (bypassing brain.py's SectorScorer.score_all) so v6.3 is fully
       independent and does NOT require changes to config.py or brain.py.
- 4.4  Direction Matrix Tracking — Every trade records index_direction,
       sector_bias, and signal_direction for post-hoc alignment analysis.
...
- 2.8  Exit Reason Taxonomy (STOP_HARD, STOP_TRAIL, TARGET1_FULL, FORCE_EXIT,
       SAFETY_HALT)
- Signal generation, grading (existing StockGrader)
- Sector selection (select_top_n from brain.py)
- Position sizing formula (RiskManager, capped by available capital)
- Microstructure gate
- Safety monitor
...
# Enable `python backtest_v6/sector_engine_v6.3.py` from repo root.
...
# ═══════════════════════════════════════════════════════════════════════════════
# V6.3 MINIMAX ALIGNMENT CONFIGURATION
# ═══════════════════════════════════════════════════════════════════════════════

# Sector scoring weights — high nifty weight forces composite sign to align
# with index direction, eliminating need for complex post-filters.
V63_SECTOR_WEIGHTS = {
    "structural": 3.0,
    "shortterm": 10.0,
    "intraday": 20.0,
    "breadth": 20.0,     # Reduced from 40 — still differentiates sectors, won't override nifty
    "nifty": 150.0,      # Raised from 30 — forces automatic alignment
}

# Bias threshold — raised to prevent over-sensitivity on tiny nifty movements.
# With nifty weight 150, a ±0.15% nifty move contributes ±22.5 to composite,
# which cleanly clears this threshold.  Moves < ±0.1% stay NEUTRAL.
V63_BIAS_THRESHOLD = 20.0
...
class BacktestTrade:
    ...
    # V6.3 additions — direction matrix tracking
    index_direction: str = ""           # UP / DOWN / FLAT (nifty direction at entry)
    sector_bias_at_entry: str = ""      # LONG / SHORT / NEUTRAL (sector bias at entry)
    signal_direction: str = ""          # LONG / SHORT (signal direction = self.direction)
...
class SectorBacktesterV6:
    """
    V6.3 backtester using V6.2 as base with MINIMAX-optimized sector scoring:
    high nifty weight (150) forces alignment, raised threshold (±20) filters
    noise.  Direction matrix tracked per trade for post-hoc analysis.
    """
    ...
    def __init__(
        ...
        self.history_dir = self.backtest_root / "history_v6.3"
    ...
    # ═══════════════════════════════════════════════════════════════════════════
    # V6.3 — MINIMAX Local Sector Scoring
    # Bypasses brain.py's SectorScorer.score_all to apply V6.3 weights and
    # threshold independently, without modifying config.py or brain.py.
    # ═══════════════════════════════════════════════════════════════════════════

    def _score_sectors_v63(
        self, sectors: List[SectorScore], nifty_pct: float
    ) -> List[SectorScore]:
        """Compute composite score, bias, and rank using MINIMAX weights."""
        if not sectors:
            return []

        w = V63_SECTOR_WEIGHTS
        threshold = V63_BIAS_THRESHOLD

        for s in sectors:
            raw_score = 0.0
            raw_score += s.structural_rs * w["structural"]
            raw_score += s.shortterm_rs * w["shortterm"]
            raw_score += s.intraday_rs * w["intraday"]
            raw_score += s.breadth * w["breadth"]
            raw_score += nifty_pct * w["nifty"]
    ...
    # ═══════════════════════════════════════════════════════════════════════════
    # V6.3 — Index Direction Helper
    # ═══════════════════════════════════════════════════════════════════════════

    @staticmethod
    def _get_index_direction(nifty_pct: float) -> str:
        """Classify current nifty % change into UP / DOWN / FLAT."""
        if nifty_pct > 0.05:
            return "UP"
        elif nifty_pct < -0.05:
            return "DOWN"
    ...
    def _capture_config_snapshot(self) -> Dict[str, Any]:
        ...
        # V6.3: MINIMAX alignment overrides
        snapshot["V6_3_SECTOR_WEIGHTS"] = V63_SECTOR_WEIGHTS
        snapshot["V6_3_BIAS_THRESHOLD"] = V63_BIAS_THRESHOLD
        return snapshot
    ...
    def _build_run_summary(self) -> Dict[str, Any]:
        ...
        # V6.3: Direction matrix breakdown
        direction_matrix = self._build_direction_matrix()
        ...
        return {
            ...
            # V6.3: direction matrix
            "direction_matrix": direction_matrix,
        }

    def _build_direction_matrix(self) -> Dict[str, Any]:
        """Build direction matrix breakdown (Index × Sector × Signal) from
        closed trades for alignment analysis."""
    ...
    def _update_sector_scores(self, ts: pd.Timestamp, baselines: Dict[str, Dict[str, float]], regime: str):
        ...
        # V6.3: Use local MINIMAX scoring instead of brain.py's score_all
        ranked = self._score_sectors_v63(scores, self.nifty_pct)
    ...
    def _process_entry(
        ...
        # V6.3: Capture index direction at this moment
    ...
    def _print_grand_summary(self):
        ...
        print("\n" + "=" * 80)
        print("V6.3 BACKTEST SUMMARY (MINIMAX ALIGNMENT)")
        print("-" * 80)
        ...
        if self.trade_history:
            ...
            # V6.3: MINIMAX Alignment — Direction Matrix
            print(f"\nMINIMAX Alignment (V6.3):")
            print(f"  Sector Weights:    nifty={V63_SECTOR_WEIGHTS['nifty']}, breadth={V63_SECTOR_WEIGHTS['breadth']}")
            print(f"  Bias Threshold:    ±{V63_BIAS_THRESHOLD}")
    ...
    def export_trades(self, filepath: Optional[Path] = None):
        out_path = Path(filepath) if filepath else (self.data_root / "backtest_trades_v6.3.csv")
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="V6.3 sector backtest engine (MINIMAX alignment)")
...
if __name__ == "__main__":
    ...
    if args.export:
        ...
        engine.export_trades(filepath=export_path)
