"""
===============================================================================
V6.6 SECTOR BACKTEST ENGINE — STRICT A+ SIGNAL FILTER
===============================================================================
This engine keeps V6.5 execution/risk/exit architecture and introduces a
strict signal model:

1) HMA alignment is now only on 5-minute data:
   - HMA(9) > HMA(21) -> bullish direction
   - HMA(9) < HMA(21) -> bearish direction
2) RVOL, StochRSI, and SpreadQuality are removed from signal grading.
3) Only Top-3 sector ranks are eligible (rank 4/5 logic removed).
4) Tradeable signal is only A+:
   - Must satisfy Top-3 + sector-bias match + index/sector/signal 3-way
     alignment (UP,LONG,LONG or DOWN,SHORT,SHORT).
   - All other cases are grade C and rejected.

Risk controls, microstructure gate, position sizing, and lifecycle management
remain unchanged from V6.5.
===============================================================================
"""
...
# Enable `python backtest_v6/sector_engine_v6.6.py` from repo root.
...
class SectorBacktesterV6:
    """
    V6.6 backtester with strict A+ signal policy:
    - 5m HMA(9)/HMA(21) direction only
    - Top-3 sector-only eligibility
    - Mandatory index/sector/signal 3-way alignment
    - No RVOL/StochRSI/SpreadQuality grading
    """
    ...
    def __init__(
        ...
        self.history_dir = self.backtest_root / "history_v6.6"
    ...
    def _capture_config_snapshot(self) -> Dict[str, Any]:
        ...
        # V6.6: strict A+ signal policy
        snapshot["V6_6_HMA_SOURCE"] = "5M_ONLY"
        snapshot["V6_6_HMA_FAST"] = 9
        snapshot["V6_6_HMA_SLOW"] = 21
    ...
    def _mark_top3_sectors(ranked_sectors: List[SectorScore]):
        """V6.6: hard-limit signal universe to top-3 non-neutral sectors."""
    ...
    def _get_hma_alignment(
        ...
        hma9 = calculate_hma(series_5m, 9)
        hma21 = calculate_hma(series_5m, 21)
        if hma9 <= 0 or hma21 <= 0:
            return "MIXED", float(hma9), float(hma21)

        if hma9 > hma21:
            return "BULLISH", float(hma9), float(hma21)
        if hma9 < hma21:
            return "BEARISH", float(hma9), float(hma21)
        return "MIXED", float(hma9), float(hma21)

    def _build_signal_v66(
        self,
        symbol: str,
        sector: str,
        price: float,
    ...
    def _scan_tradeable_stocks(
        ...
        for scan_order, symbol in enumerate(candidates, start=1):
            ...
            hma_align, hma9_5m, hma21_5m = self._get_hma_alignment(symbol, curr_price, intra_pos)
            ...
            signal.hma9_5m = float(hma9_5m)
            signal.hma21_5m = float(hma21_5m)
    ...
    def _process_entry(
        ...
        # Full opportunity universe: all A+ signals and their potential position/trade data.
    ...
    def _print_grand_summary(self):
        ...
        print("\n" + "=" * 80)
        print("V6.6 BACKTEST SUMMARY (STRICT A+ SIGNAL FILTER)")
        print("-" * 80)
        ...
        if self.trade_history:
            ...
            # V6.4/V6.6: Hard Alignment Filter — Direction Matrix
            print(f"\nAlignment Filter (V6.4/V6.6 — Hard Block):")
    ...
    def export_trades(self, filepath: Optional[Path] = None):
        out_path = Path(filepath) if filepath else (self.data_root / "backtest_trades_v6.6.csv")
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="V6.6 sector backtest engine (strict A+ signal filter)")
