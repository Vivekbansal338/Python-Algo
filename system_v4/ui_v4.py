...
# ══════════════════════════════════════════════════════════════════════════════
# STYLE CONSTANTS (Imported from V3 logic)
# ══════════════════════════════════════════════════════════════════════════════
...
class DashboardUI:
    ...
    def get_session_style(self, session: str) -> tuple:
        # Map V4 session strings to styles
        styles = {
    ...
    def generate_header(self, session_name: str, regime: str, vix: float, nifty_val: float, nifty_pct: float) -> Panel:
        ...
        return Panel(
            Align.center(header),
            title="[bold cyan]📊 V4 AUTONOMOUS BOT[/]",
            border_style="cyan",
            box=box.DOUBLE,
            padding=(0, 1)
        )
    ...
    def generate_scanner(self, sectors: list, stocks: list, nifty_pct: float = 0.0, active_positions: list = None) -> Panel:
        ...
        sec_table.add_column("Bias", justify="center", width=5) # New Bias Column
        sec_table.add_column("Price", justify="right", width=8)
        sec_table.add_column("Chg%", justify="right", width=6)
        ...
        for s in sectors[:12]: # Show Top 8 to fit detailed view
            ...
            # Name (Truncated more aggressively)
        ...
        for rank, s in enumerate(sorted_stocks[:15], 1): # Increased from 8 to 15
            ...
            # Spread % Style (relative to gate threshold)
            sprd_pct = s.spread_atr * 100
            sprd_style = "bright_green" if sprd_pct < 10 else "yellow" if sprd_pct < 20 else "bright_red"

            # Determine Sector Bias display from stock direction match
            # We don't have sec_bias stored directly on signal, but we can infer or pass it.
            # Best way: Check if Grade C + Reason "Sector Bias"
            bias_disp = "NEUT"
            bias_style = "dim"
            
            # Simple inference: check if score > 0 (A/B) -> Matched
            if s.grade in ["A+", "A", "B"]:
                bias_disp = s.direction[:4] # LONG/SHOR
                bias_style = "bright_green" if s.direction == "LONG" else "bright_red"
            else:
                # If disqualified due to bias, show the bias that blocked it
                if any("Sector Bias" in r for r in s.reasons):
                   bias_disp = "OPP" # Opposite
                   bias_style = "bright_red"
                else:
                   bias_disp = "-"
    ...
    def generate_logs(self, logs: list) -> Panel:
        ...
        for log in logs[-10:]:
            ...
            if "ENTER" in log: style = "bright_cyan"
            elif "EXIT" in log: style = "bright_yellow"
            elif "STOP" in log: style = "bright_red"
            elif "TARGET" in log: style = "bright_green"
            else: style = "grey70"
    ...
    def update(self, state_data: dict):
        ...
        self.layout["footer"].update(Panel(Align.center(f"MODE: {mode} | V4.0.0"), style="grey50"))
